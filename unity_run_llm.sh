...
script_path="ViCo/meeting_challenge/scripts/run_llm.sh"
...
for scene in "${scenes[@]}"; do
  for task in "${task_types[@]}"; do
    ...
    salloc -p gpu-preempt -G 1 --mem=100G -t 240 --job-name=llm_$scene --constraint="vram80|a40|l40s" srun bash "$script_path" "$scene" &
