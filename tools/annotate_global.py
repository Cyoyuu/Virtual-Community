...
def overlay_locations_desp_on_image():
    ...
    font_size = 14
    padding=2
    font=ImageFont.truetype("ViCo/assets/arial.ttf", font_size)
...
def get_building_to_places():
    ...
    with open(f"ViCo/assets/scenes/{args.scene}/raw/inaccessible_buildings.json", 'r') as f:
        inaccessible_buildings = json.load(f)

    with open(f"ViCo/assets/scenes/{args.scene}/raw/building_to_places.json", 'r') as f:
        building_to_places = json.load(f)

    with open(f"ViCo/assets/scenes/{args.scene}/place_metadata.json", 'r') as f:
        place_metadata = json.load(f)

    with open(f"ViCo/assets/scenes/{args.scene}/building_metadata.json", 'r') as f:
