...
scenes=("MADRID" "HARVARD" "AMSTERDAM" "AUSTIN" "BERLIN" "CALGARY" "COLUMBUS" "DENVER" "DETROIT" "EL_PASO" "HAMBURG" "LONGISLAND" "MADISON" "BARCELONA" "LONDON" "FRANKFURT")
...
for scene in "${scenes[@]}"; do
  for task in "${task_types[@]}"; do
    ...
    bash "$script_path" "$scene" "$gt" "$agent_num" "$sentinel_type" "$sentinel_num" &
