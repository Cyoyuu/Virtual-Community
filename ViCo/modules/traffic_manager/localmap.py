    ...
    def __init__(self, file_path, terrain_height_path, ref_lat, ref_lon):
        ...
        for road in root.findall('road'):
                ...
                lanes.append(
                    {
                        ...
                        'width': (float(lane.find('width').get('a'))+float(lane.find('roadMark').get('width')))*1.4,
                        'link': {
                            'predecessor': [],
                            'successor': []
                        }
            ...
            if predecessor.get('elementType')=='road':
                prev_roads = [predecessor.get('elementId')]
            else:
                if predecessor.get('elementId') not in self.junctions: return []
                junc = [predecessor.get('elementId')]
            ...
            if successor.get('elementType')=='road':
                next_roads = [successor.get('elementId')]
            else:
                if successor.get('elementId') not in self.junctions: return []
                junc = self.junctions[successor.get('elementId')]
