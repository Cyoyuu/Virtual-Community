...
def wrap_coord(v, thresh=500.0, offset=1000.0):
    if v > thresh:
        return v - offset
    if v < -thresh:
        return v + offset
...
def wrap_xy(x, y, thresh=500.0, offset=1000.0):
    return wrap_coord(x, thresh, offset), wrap_coord(y, thresh, offset)
...
def precompute_histories(full_steps, steps_data, agents, max_jump=100.0):
    ...
    agent_traces = {
        name: {"xs": [None] * len(full_steps), "ys": [None] * len(full_steps)}
        for name in agents
    }
        ...
        for obs in obs_list:
            ...
            if name in agent_traces:
                ...
                if isinstance(pose, (list, tuple)) and len(pose) >= 2:
                    x, y = float(pose[0]), float(pose[1])
                    x, y = wrap_xy(x, y)
                    agent_traces[name]["xs"][i] = x
                    agent_traces[name]["ys"][i] = y
...
def animate_all(
    ...
    # 从新的 patrol 目录读数据
    base_dir = f"/scratch4/workspace/xiangyelin_umass_edu-simple/cvpr_experiments/{scene}/sentinel_no_gt_5/patrol_10/job_1"
    ...
    # 真实数据的最大 step
    max_step_in_data = step_ids[-1]

    # 实际绘制的结束步数不能超过数据范围
    end_step = min(max_steps, max_step_in_data)

    # 构造真正用来画的 full_steps / frames
    full_steps = list(range(start_step, end_step + 1))
    frames = full_steps[::5]

    print(
        f"[INFO] steps.json step range: min={step_ids[0]}, max={step_ids[-1]}, "
        f"draw range: {start_step} -> {end_step}, frames={len(frames)}"
    )

    agents_all, sentinels = build_entity_lists(steps_data, track_sentinels=False)
    print("[DEBUG] num agents =", len(agents_all), "example:", agents_all[:5])
    print("[DEBUG] num sentinels =", len(sentinels), "example:", sentinels[:5])
    ...
    agent_traces, idx_of_full = precompute_histories(
        full_steps, steps_data, fixed_agents, max_jump=20.0
    )
    ...
    agent_dots = {}
    # 图例标签为 Agent 1–5
    for idx, name in enumerate(fixed_agents):
        color = agent_color[name]
        label = f"Agent {idx + 1}"
        (line,) = ax.plot([], [], linewidth=2.5, color=color, label=label, zorder=2)
    ...
    bus_scat = ax.scatter(
        [], [], s=55, marker="s", color="red", label="Bus (now)", zorder=5
    )

    sentinel_legend_proxy, = ax.plot(
        [],
        [],
        linestyle="None",
        marker=(3, 0, 0),
    ...
    def update(step):
        ax.set_title(
            f"{scene}: Agents history & Sentinels/Bus live positions — step {step}"
        )
        ...
        for sname in sentinels:
            ...
            if pos_now is not None:
                ...
                if prev is not None and (
                    pos_now[0] != prev[0] or pos_now[1] != prev[1]
                ):
            ...
            if eff_pos is not None:
                ...
                ang = heading_from(
                    sentinel_prev_pos.get(sname, None), eff_pos, default_ang
                )
                ...
                for aname in fixed_agents:
                    ...
                    if axc is None or ayc is None:
                        continue
                    if in_sector(
                        eff_pos[0], eff_pos[1], r, theta1, theta2, axc, ayc
                    ):
                        alert = True
                        break
                wedge.set_facecolor("red" if alert else "#FFF3B0")
            else:
                wedge.set_visible(False)
                tri.set_data([], [])
    ...
    if out_format.lower() == "mp4":
        ...
        for codec in ["libx264", "mpeg4"]:
            ...
            raise last_err if last_err else RuntimeError(
                "FFMpegWriter failed with all codecs."
            )
